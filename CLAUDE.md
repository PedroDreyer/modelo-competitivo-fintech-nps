...
El modelo tiene 2 tareas que SIEMPRE requieren al agente:
...
7. **Re-ejecutar el modelo** con `python correr_modelo.py`

### PASO 2B: BUSCAR NOTICIAS (SIEMPRE, hecho por el agente)

**CRÍTICO:** La búsqueda de noticias la hace el agente con WebSearch, NO el script Python.
El script Python falla al scrapear motores de búsqueda (DDG/Bing bloquean).

**Después de generar las causas raíz (paso 5), el agente DEBE:**

1. **Leer las causas raíz** del JSON que acaba de generar
2. **Para cada motivo con causas raíz**, buscar noticias con WebSearch:
...
**FORMATO DEL JSON de causas raíz:**
```json
{
  "metadata": {
    "player": "Nubank",
...
### 2. NOTICIAS SIEMPRE LAS BUSCA EL AGENTE (NO el script Python)

La búsqueda de noticias la hace **el agente con WebSearch**, NO el script Python.
...
- 2da ejecución → modelo lee cache → triangula → HTML
...
- `data/noticias_cache.json` (cache de noticias)
