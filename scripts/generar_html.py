...
def _generar_promotores_causas_content(resultados, q_ant, q_act, player):
    ...
    causas_por_motivo = promotores_data['causas_por_motivo']
    ...
    html = f"""
        <div style="background: linear-gradient(135deg, #065f46 0%, #10b981 100%); padding: 25px; border-radius: 12px; color: white; margin-bottom: 25px; margin-top: 25px;">
            <div style="font-size: 14px; opacity: 0.9; margin-bottom: 8px;">🌟 Deep Dive: Causas de Satisfacción</div>
            <div style="font-size: 24px; font-weight: 700; margin-bottom: 5px;">{player} - {q_ant} → {q_act}</div>
            <div style="opacity: 0.85; font-size: 13px;">{len(causas_por_motivo)} motivos analizados · Basado en comentarios reales de promotores</div>
        </div>
    """
    ...
    for motivo, datos in motivos_ordenados:
        ...
        if delta > 1:
            header_bg = 'linear-gradient(135deg, #059669 0%, #10b981 100%)'
            delta_label = f'🔥 +{delta:.1f}pp (creció)'
        elif delta > 0:
            header_bg = 'linear-gradient(135deg, #10b981 0%, #34d399 100%)'
            delta_label = f'✅ +{delta:.1f}pp'
        elif delta > -1:
            header_bg = 'linear-gradient(135deg, #6b7280 0%, #9ca3af 100%)'
            delta_label = f'➡️ {delta:.1f}pp (estable)'
        else:
            header_bg = 'linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%)'
            delta_label = f'⚠️ {delta:.1f}pp (bajó)'

        motivo_id = 'dp_' + motivo.lower().replace(' ', '_').replace('á', 'a').replace('é', 'e').replace('í', 'i').replace('ó', 'o').replace('ú', 'u')
        ...
        html += f"""
            <div style="background: white; border-radius: 12px; margin-bottom: 20px; border: 1px solid #e5e7eb; overflow: hidden;">
                <div onclick="document.getElementById('{motivo_id}').style.display = document.getElementById('{motivo_id}').style.display === 'none' ? 'block' : 'none'; this.querySelector('.chevron').textContent = document.getElementById('{motivo_id}').style.display === 'none' ? '▶' : '▼';"
                    ...
                    <div>
                        <div style="font-size: 17px; font-weight: 700;">{motivo}</div>
                        <div style="font-size: 12px; opacity: 0.9; margin-top: 4px;">{delta_label} · {total_comms} comentarios · {len(causas)} fortalezas identificadas</div>
                    </div>
                    <span class="chevron" style="font-size: 16px; opacity: 0.8;">▶</span>
                </div>
        ...
        colors = ['#10b981', '#059669', '#14b8a6', '#0d9488']
        for i, causa in enumerate(causas):
            titulo = html_module.escape(causa.get('titulo', ''))
            desc = html_module.escape(causa.get('descripcion', ''))
            freq_pct = causa.get('frecuencia_pct', 0)
            ...
            html += f"""
                    <div style="margin-bottom: 20px; padding: 16px; background: #f0fdf4; border-radius: 10px; border-left: 4px solid {bar_color};">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
                            <div style="font-weight: 700; font-size: 14px; color: #1f2937; flex: 1;">{titulo}</div>
                            <div style="background: {bar_color}; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; white-space: nowrap; margin-left: 12px;">
                                {freq_pct}% ({freq_abs})
                        ...
                        <div style="background: #d1fae5; border-radius: 4px; height: 6px; margin-bottom: 10px; overflow: hidden;">
                            <div style="background: {bar_color}; width: {min(freq_pct, 100)}%; height: 100%; border-radius: 4px;"></div>
                        </div>
                        <div style="color: #4b5563; font-size: 13px; line-height: 1.5; margin-bottom: 10px;">{desc}</div>
                        <div style="margin-top: 8px;">
                            <div style="font-size: 11px; color: #059669; font-weight: 600; margin-bottom: 6px; text-transform: uppercase;">Comentarios representativos</div>
            """

            for ej in ejemplos[:3]:
                ej_escaped = html_module.escape(str(ej))
                html += f"""
                            <div style="background: white; border: 1px solid #d1fae5; padding: 10px 14px; border-radius: 8px; margin-bottom: 6px; font-style: italic; color: #374151; font-size: 12px; line-height: 1.4;">
                                "{ej_escaped}"
                            </div>
                """
